...
class AnalysisArgs:
    ...
    trend_station_ids: List[str]
    trend_time_start: Optional[float]
    trend_time_end: Optional[float]
    trend_y_ticks: List[float]
    keep_missing: bool
...
def parse_args() -> Tuple[AnalysisArgs, object, dict]:
    ...
    parser.add_argument("--trend_station_ids", type=str, default="", help="趋势图站点列表，逗号分隔")
    parser.add_argument("--trend_time_start", type=str, default="", help="趋势图时间范围起点（时间字符串或时间戳）")
    parser.add_argument("--trend_time_end", type=str, default="", help="趋势图时间范围终点（时间字符串或时间戳）")
    parser.add_argument("--trend_y_ticks", type=str, default="", help="趋势图纵轴刻度，逗号分隔")
    ...
    analysis_args = AnalysisArgs(
        ...
        trend_station_ids=[s.strip() for s in str(analysis_ns.trend_station_ids).split(",") if s.strip()],
        trend_time_start=parse_time_value(analysis_ns.trend_time_start),
        trend_time_end=parse_time_value(analysis_ns.trend_time_end),
        trend_y_ticks=[float(s.strip()) for s in str(analysis_ns.trend_y_ticks).split(",") if s.strip()],
...
def generate_csv_and_stats_flat(
    y_pred: np.ndarray,
    y_target: np.ndarray,
    time_arr: np.ndarray,
    missing_target: Optional[np.ndarray],
    ...
    """Write row-wise chunked inference results and compute aggregate metrics."""
    if os.path.exists(csv_path):
        os.remove(csv_path)
    ...
    if y_pred.ndim != 2 or y_target.ndim != 2:
        raise ValueError(f"flat results require 2D arrays, got y_pred={y_pred.shape}, y_target={y_target.shape}")

    time_arr = np.asarray(time_arr).reshape(-1)
    target_node_index = np.asarray(target_node_index).reshape(-1).astype(int)
    missing_target = None if missing_target is None else np.asarray(missing_target)
    if missing_target is not None:
        if missing_target.ndim == 1:
            missing_target = missing_target[:, np.newaxis]
        if missing_target.ndim != 2:
            raise ValueError(f"flat missing_target must be 1D/2D, got {missing_target.shape}")

    n_rows = min(len(time_arr), len(target_node_index), y_pred.shape[0], y_target.shape[0])
    n_feat = min(y_pred.shape[1], y_target.shape[1])
    print(f"[CSV] 分块结果维度：rows={n_rows}, feature={n_feat}")
    ...
    for row_idx in range(n_rows):
        timestamp = float(time_arr[row_idx])
        time_str = format_timestamp(timestamp)
        node_idx = int(target_node_index[row_idx])
        station_id = station_ids[node_idx] if 0 <= node_idx < len(station_ids) else f"NODE_{node_idx:03d}"

        for f_idx in range(n_feat):
            true_val = float(y_target[row_idx, f_idx])
            pred_val = float(y_pred[row_idx, f_idx])
            abs_error = abs(pred_val - true_val)
            sq_error = (pred_val - true_val) ** 2
            err = pred_val - true_val
            missing_flag = False
            if missing_target is not None:
                miss_col = min(f_idx, missing_target.shape[1] - 1)
                missing_flag = bool(missing_target[row_idx, miss_col] > 0.5)
        ...
        if (row_idx + 1) % max(1, n_rows // 10) == 0:
            print(f"[CSV] 已处理记录 {row_idx + 1}/{n_rows}")
    ...
    return summary, station_metrics_df, pd.DataFrame(sample_rows), count_info
...
def collect_trend_data_from_csv(
    ...
    for chunk in pd.read_csv(csv_path, chunksize=200000):
        if time_start is not None:
            chunk = chunk[chunk["timestamp"] >= time_start]
        if time_end is not None:
            chunk = chunk[chunk["timestamp"] <= time_end]
...
def generate_trend_plot(
    ...
    if not station_order:
        ...
        return []
    if top_k > 0:
        station_order = station_order[:top_k]
...
def write_report(
    ...
    if analysis_args.trend_station_ids:
        lines.append(f"趋势图筛选站点: {', '.join(analysis_args.trend_station_ids)}")
    if analysis_args.trend_time_start is not None or analysis_args.trend_time_end is not None:
        start_text = format_timestamp(analysis_args.trend_time_start) if analysis_args.trend_time_start is not None else "-"
        end_text = format_timestamp(analysis_args.trend_time_end) if analysis_args.trend_time_end is not None else "-"
        lines.append(f"趋势图时间范围: {start_text} ~ {end_text}")
    if trend_plot_paths:
...
def main() -> None:
    ...
    target_node_index = np.asarray(results["target_node_index"]).reshape(-1) if "target_node_index" in results else None
    if target_node_index is not None:
        print(f"target_node_index shape: {target_node_index.shape}")

    print("[4/6] 构建站点映射")
    flat_chunked_results = target_node_index is not None and y_pred.ndim <= 2
    if flat_chunked_results:
        max_node_index = int(np.max(target_node_index)) if len(target_node_index) > 0 else 0
        station_ids, station_meta_source = load_station_id_table(analysis_args, opt, dataset, max_node_index)
        target_nodes = np.unique(target_node_index.astype(int))
        mapping_source = "<results.target_node_index>"
        station_mapping = []
        print(f"[映射] 使用分块结果中的 target_node_index，唯一目标站点={len(target_nodes)}")
    else:
    ...
    if flat_chunked_results:
        summary, station_metrics_df, sample_df, count_info = generate_csv_and_stats_flat(
            y_pred=y_pred,
            y_target=y_target,
            time_arr=time_arr,
    ...
    print("[6/6] 绘图并输出报告")
    selected_stations = analysis_args.trend_station_ids
    if not selected_stations and not station_metrics_df.empty:
    ...
    trend_df = collect_trend_data_from_csv(
        ...
        time_start=analysis_args.trend_time_start,
        time_end=analysis_args.trend_time_end,
    )
    trend_plot_paths = generate_trend_plot(
        trend_df,
        trend_plot_path,
        analysis_args.trend_top_k_stations,
        use_cn_font,
        analysis_args.trend_y_ticks,
    )
